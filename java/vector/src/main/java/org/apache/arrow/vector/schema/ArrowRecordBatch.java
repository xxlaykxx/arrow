...
public class ArrowRecordBatch implements ArrowMessage {
   ...
   */
  public ArrowRecordBatch(int length, List<ArrowFieldNode> nodes, List<ArrowBuf> buffers, boolean allignBuffers) {
    super();
    this.length = length;
    this.nodes = nodes;
    ...
    for (ArrowBuf arrowBuf : buffers) {
      ...
      offset += size;
      if (allignBuffers && offset % 8 != 0) { // align on 8 byte boundaries
        offset += 8 - (offset % 8);
      }
    }

...
public final class BitVector extends BaseDataValueVector implements FixedWidthVector {
  ...
  public void splitAndTransferTo(int startIndex, int length, BitVector target) {
    ...
    int lastByteSource = getByteIndex(valueCount);
    ...
    if (length > 0) {
      if (offset == 0) {
        ...
        int destPos = 0;
        int sourcePos = startIndex;
        // TODO maybe do this one word at a time, rather than byte?

        int counter = firstByteSource;

        for (int i = 0; i < byteSizeTarget - 1; i++) {
          target.data.setByte(i,
                  (((this.data.getByte(firstByteSource + i) & 0xFF) >>> offset) + (this.data.getByte(firstByteSource + i + 1) << (8 - offset))));
          counter++;
        }

        /* Copying the last piece is done in the following manner:
         * if the source vector has 1 or more bytes remaining, we copy the last piece as a byte formed by shifting data
         * from the current byte and the next byte.
         *
         * if the source vector has no more bytes remaining (we are at the last byte), we copy the last piece as a byte
         * by shifting data from the current byte.
         *
         */
        if((counter < lastByteSource)) {
          target.data.setByte(byteSizeTarget - 1,
                  (((this.data.getByte(firstByteSource + byteSizeTarget - 1) & 0xFF) >>> offset) +
                          (this.data.getByte(firstByteSource + byteSizeTarget) << (8 - offset))));
        }
        else {
          byte b = (byte)(((this.data.getByte(firstByteSource + byteSizeTarget - 1) & 0xFF) >>> offset));
          target.data.setByte(byteSizeTarget - 1, b);
        }
      }
    }
